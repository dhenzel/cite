...
**Human website and buyer MCP name the operator.** *(2026-08-18, expanded 2026-08-20.)* People pay real money. A domain with no company reads as a fly-by-night. The placement.sh wordmark uses Shortlist’s mint period (`placement.sh`). HTML explains that a named Shortlist team has bought publisher placements since 2018; Who runs this leads to [the team](https://shortlist.io/about-us/), then a [15-min call](https://calendly.com/shortlist-businessdevelopment/15min), then [shortlist.io](https://shortlist.io/). Claude/MCP `help`, initialize instructions, and payment `next_step` tell the agent to show those same three links in the chat before the human pays — including offering the call if they want to talk to someone.
